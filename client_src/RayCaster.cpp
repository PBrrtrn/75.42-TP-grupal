...
RayCaster::RayCaster(Map& map) : map(map) { }
...
int RayCaster::getWallTextureAt(Vector position,Vector direction){
	...
	if (y_dir < 0)
		...
		*/
	//std::cout << "celltexture  :" << x << "," << y  << std::endl;
	return 0; //this->map.getGridValue(x,y);
}

bool RayCaster::findCollision(float x_i,float y_i,float x_dir,float y_dir,float& x_c,float& y_c,int& wall_texture){
	...
	if (y_dir < 0)
		y = ((int)(y_i - 1)/CELL_HEIGHT)%this->map.getHeight();
	if (this->map.getGridValue(x,y) != 0 ){
		wall_texture = this->map.getGridValue(x,y);
		return false;
	}
		
	//std::cout << "cellCollision:" << x << "," << y  << std::endl;
...
Vector RayCaster::getIntersection(Vector playerPosition,
                                  Vector playerDirection,int& wall_texture) {
    ...
    int fin = 0;
    while (findCollision(x_c,y_c,playerDirection.getXCoordinate(),playerDirection.getYCoordinate(),x_c,y_c,wall_texture) && fin < (this->map.getWidth())){
		fin++;
	}
...
float RayCaster::getWallHeight(Vector playerPosition, Vector playerDirection,
    ...
    Vector v = getIntersection(playerPosition, playerDirection,wall_texture);
    playerDirection.turnUnitary();

    Vector v2 = v + (playerPosition * -1);
    float distance = v2.norm();
    ...
    wall_texture = this->getWallTextureAt(v,playerDirection);
		...
		std::cout << "pos:" << v.getXCoordinate() << "," << v.getYCoordinate()  << std::endl;
